...
if __name__ == "__main__":
    ...
    folder = os.path.join(here, "frames")              
    ...
    for name in os.listdir(folder):
        ...
        img = np.asarray(img_raw).astype(np.float32) / 255.0  # [0,1]
        # ds = 2
        # small = cv2.resize(img, (img.shape[1]//ds, img.shape[0]//ds),
        #                        interpolation=cv2.INTER_AREA)
        t_total_start = time.perf_counter()

        t_enh_start = time.perf_counter()
        enhanced_img, R, L = enhance_image(img)
        t_enh_end = time.perf_counter()

        t_total_end = time.perf_counter()
        
        simple_enhanced = simple_enhance(full, simple_path)
        gamma_enhanced = simple_gamma(full, gamma_path)
        ...
        # mask_full = cv2.resize(small, (img.shape[1], img.shape[0]),
        #                            interpolation=cv2.INTER_NEAREST)
        
        # mask_R = cv2.resize(R, (img.shape[1], img.shape[0]),
        #                            interpolation=cv2.INTER_NEAREST)
        Image.fromarray(enhanced_img).save(out_path)
        Image.fromarray(R).save(R_path)
        Image.fromarray(L).save(L_path)
